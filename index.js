...
io.on('connection', (socket) => {
    ...
    socket.on('addsesiondata', function(data) {
        ...
        fs.readFile(therapyConfigPath, (err, data) => {
            ...
            con.query(IDs , [1,2], function (err, result) {
				...
				console.log(patient_leg_length);
    ...
    socket.on('games:download_sessions_objectives',function(res){
        ...
        worksheet.columns = [
            ...
            { header: 'Posicion X', key: 'posicion_x', width: 10 },
            { header: 'Posicion Y', key: 'posicion_y', width: 10 },
            { header: 'Posicion Z', key: 'posicion_z', width: 10 },
            { header: 'Pixel X', key: 'pixel_x', width: 10 },
            { header: 'Pixel Y', key: 'pixel_y', width: 10 },
            { header: 'Screen Width', key: 'sw', width: 20 },
            { header: 'Screen Height', key: 'sh', width: 20 },
            { header: 'Tiempo de Alcnce', key: 'tiempo', width: 20 },
...
var anglesRotation_vector;
var anglesFlexion_vector;
var id_user_playing;
...
io_games.on('connection', (socket) => {
    ...
    socket.on("message", (data) => {
		...
		try {
			...
			} else if (receivedJSON.cabecera == 'games:angles'){
...
function DDBB_addObjectivesData(objectives_data){
	...
	let n_objetivo = objetives_data.n_objetivo;
...
function DDBB_addSessionData(){
	...
	for (let index = 0; index < anglesFlexion_vector.length; index++) {
		...
		var sql = "INSERT INTO tabla_sesiones (id_sesion, Date, alfa, beta, gamma, posX, posY, posZ) VALUES " + insertDataRows;
		con_games.query(sql, function (err, result) {
			if (err) throw err;
		});
	}	
...
function moveManually(data) {  
    ...
    .catch((err) => console.log('Error en l.', err))
}
