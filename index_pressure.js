			...
			// get the entire message from the received data ('#DCM= arg1, arg2...., arg10')
			 ...
			 for(i=0; i<msg_list.length; i++){
				if(msg_list[i].includes("=") & msg_list[i].includes(',')){
					let data_vector = msg_list[i].split('=')[1].split(',');
					//if(i != (msg_list_pressure.length-1)){
					if(data_vector.length == 10){
						lasthex_imu1 = "";
						dcm_value1 = data_vector[0]
						dcm_value2 = data_vector[1]
						dcm_value3 = data_vector[2]
						dcm_value4 = data_vector[3]
						dcm_value5 = data_vector[4]
						dcm_value6 = data_vector[5]
						dcm_value7 = data_vector[6]
						dcm_value8 = data_vector[7]
						dcm_value9 = data_vector[8]
						
						// store the data message into "dcm_msgData" variable, which will be sent to the client socket games.
						dcm_msgData = "#" + msg_list[i];	
			...
			if(data_vector.length == 2){
				...
				if(record_therapy){
					if(is_imu1_connected){
						pressure_values.push([dcm_value1, dcm_value2, dcm_value3, dcm_value4, dcm_value5, dcm_value6, dcm_value7, dcm_value8, dcm_value9, data_vector[0]])			
					}
				}
			} else {
...
/
    ...
    * */
    ...
    socket.on('pressure:download', function(callbackFn) {
        ...
        worksheet.addRow(["DCM 1", "DCM 2", "DCM 3", "DCM 4", "DCM 5", "DCM 6", "DCM 7", "DCM 8", "DCM 9", "Sensor Presión"]);
        for (var i = 0; i < pressure_values.length; i++) {
			worksheet.addRow((pressure_values[i]));
		}
...
function hex2a_general_(hexx, items, lasthex, is_first_data) {
				...
				if(deviceNotFound){
					...
					if ( str_device == 'sw'){
						if (devices[i].name == swBluetoothName){
							console.log("[Bt] SW Device found. Trying connection...")
